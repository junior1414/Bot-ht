...
def enviar_telegram(mensagem):
    ...
    try:
        ...
        print(f"⚠️ Erro ao enviar Telegram: {e}", flush=True)
...
def executar():
    ...
    try:
        ...
        if '1' in sports:
            print(f"📡 Checando jogos ativos... {time.strftime('%H:%M:%S')}", flush=True)
            ...
            print("📭 Nenhum jogo de futebol ao vivo encontrado no momento.", flush=True)

    except Exception as e:
        if not conexao_perdida:
            print(f"⚠️ Erro de conexão: {e}. Tentando reconectar...", flush=True)
...
if __name__ == "__main__":
    print("🚀 BOT DE PRESSÃO (4 CHUTES / 2 NO ALVO) INICIADO...", flush=True)
    while True:
        try:
            executar()
        except Exception as e:
            print(f"💥 Erro crítico no loop: {e}", flush=True)
        time.sleep(20)